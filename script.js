...
/* =========================================
   DRESS CODE - LANGUAGE + REVEAL
   ========================================= */

(function () {

  const image = document.getElementById("dressCodeImage");
  const card = document.querySelector(".dress-code-card");
  const button = document.getElementById("dressCodeReveal");

  const eyebrow = document.querySelector("[data-dress-eyebrow]");
  const title = document.querySelector("[data-dress-title]");

  if (!image || !card || !button) return;
  ...
  function updateDressCode() {
    ...
    if (language === "hi") {
      ...
      image.src = "dress-code-hindi.png";

      eyebrow.textContent = "ड्रेस कोड";
      title.textContent = "क्या पहनें";
      button.textContent = "✨ ड्रेस कोड देखें";

    } else {

      image.src = "dress-code-english.png";

      eyebrow.textContent = "THE DRESS CODE";
      title.textContent = "What to wear";
      button.textContent = "✨ Tap for Dress Codes";
    }

    /* Start blurred again whenever language changes */
  ...
  /* Watch for English ↔ Hindi changes */
  const dressLanguageObserver = new MutationObserver(function () {
    updateDressCode();
  });

  dressLanguageObserver.observe(document.body, {
    attributes: true,
